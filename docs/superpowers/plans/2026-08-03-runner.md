...
- **Pozor na zástupné vzory nette/testeru:** `%a%` matchuje cokoliv **kromě odřádkování**, `%A%` včetně něj. Obojí ale vyžaduje **aspoň jeden znak** — na začátku nebo na konci hlášky, kde nic nepředchází ani nenásleduje, patří `%a?%` / `%A?%` (nula a víc). A nikdy neohýbej hlášku proto, aby vyhověla vzoru.
...
- Produces:
  ...
  - veřejné readonly `string $command`, `list<string> $args`
...
- Produces:
  ...
  - `Donut\Runner\ProcessRunner::run(string $command, list<string> $args, string $stdin, bool $captureStderr, ?int $timeout): ProcessResult`
...
**Files:**
- Modify: `src/Runner/Runner.php` — rozšířit `runSteps()` o dvě větve, přidat jednu privátní metodu a ošetřit nespuštěný proces
- Test: `tests/Donut/Runner.branching.phpt`
- Test: `tests/Donut/Runner.startFailure.phpt`

**Pozn. k rozsahu:** kromě `if` a `foreach` je v tomhle tasku ještě jedna
malá věc — ošetření procesu, který se vůbec nespustí. Patří sem proto, že se
mění tatáž metoda; oddělovat to do vlastního tasku by znamenalo sahat do
`runCommand()` dvakrát.
...
- [ ] **Step 7: Napsat padající test na nespuštěný proces**

Když `command` kamene na stroji neexistuje, `Process` hodí
`ProcessFailedException` dřív, než vznikne jakýkoliv exit code. Bez ošetření
dostane uživatel syrovou výjimku bez cesty ke kroku — v třicetikrokovém
workflow netuší, který krok to byl. Validátor to chytit nemůže, protože
neví, co je na `PATH`.

`tests/Donut/Runner.startFailure.phpt`:
...
Pozn.: hláška obsahuje důvod od operačního systému, který se liší podle
platformy, takže se tvrdí `%A%` kolem tří věcí, na kterých záleží — cesta
ke kroku, jméno kamene a jméno příkazu.

- [ ] **Step 8: Spustit test, ověřit že padá**

Run: `vendor/bin/tester -C tests/Donut/Runner.startFailure.phpt`
Expected: FAIL — proleze `Nette\Utils\ProcessFailedException` místo `RunFailedException`.

- [ ] **Step 9: Ošetřit nespuštěný proces**
...
A do `runCommand()` přidej **druhý** `catch` k tomu stávajícímu, hned za něj:
...
Jméno příkazu je v hlášce právě tady, protože tady je příčinou. U selhání
podle exit codu by bylo šumem — kámen si příkaz drží ve svém JSON souboru.

- [ ] **Step 10: Spustit test, ověřit že prochází**

Run: `vendor/bin/tester -C tests/Donut/Runner.startFailure.phpt`
Expected: PASS

- [ ] **Step 11: Ověřit celou sadu a PHPStan**
...
```bash
git add src/Runner/Runner.php tests/Donut/Runner.branching.phpt tests/Donut/Runner.startFailure.phpt
git commit -m "Runner: kroky if a foreach, ošetření nespuštěného procesu"
```
